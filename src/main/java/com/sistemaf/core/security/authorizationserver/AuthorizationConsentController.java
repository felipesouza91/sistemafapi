...
public class AuthorizationConsentController {
  ...
  public String consent(Principal principal, Model model,
    ...
    Set<String> scopeToApproval = new HashSet<>(Set.of(scopeArray));
    Set<String> scopesLastApproval;
    if( consent != null ) {
      scopesLastApproval = consent.getScopes();
      scopeToApproval.removeAll(scopesLastApproval);
    } else {
      scopesLastApproval = Collections.emptySet();
    }
    model.addAttribute("clientId", clientId);
    model.addAttribute("state", state);
    model.addAttribute("principalName", principal.getName());
    model.addAttribute("scopesToApproval", scopeToApproval);
    model.addAttribute("scopesLastApproval", scopesLastApproval);
